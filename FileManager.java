...
public static ArrayList<Complaint> loadManagerComplaintsFromFile(Dept department) {
    ...
    try {
        ...
        if (!file.exists()) {
            System.out.println(ConsoleInterface.ANSI_RED+"No complaints filed for this department ");
...
public static ArrayList<Employee> loadManagerEmployeesFromFile(Dept department) {
    ...
    try {
        ...
        if (!file.exists()) {
            System.out.println(ConsoleInterface.RED_BOLD+"No employees in your department yet ");
...
public static ArrayList<Job> loadAssignmentsForEmployee(String employeeUsername) {
    ...
    if (!file.exists()) {
        System.out.println(ConsoleInterface.ANSI_RED + "No Assignments for you yet");
        return assignments;  // Exit the method if the file doesn't exist
    }
...
public static void updateAssignmentStatus(Job assignment) {
    ...
    try (PrintWriter writer = new PrintWriter(new FileWriter(filePath))) {
        ...
        for (String line : lines) {
            writer.println(line); // This adds a newline after each line
        }
        //writer.println(); // Add an additional newline after all lines are printed
    } catch (IOException e) {
        e.printStackTrace();
    }
...
public static String viewAssignedComplaintSolutions(int complaintId) {
    ...
    if (!file.exists()) {
        System.out.println(ConsoleInterface.ANSI_RED + "Complaint not assigned yet.");
...
public static String getDescriptionByCid(int targetCid, String filePath) {
    ...
    try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
        ...
        while ((line = reader.readLine()) != null) {
            ...
            if (cid == targetCid) {
                //description = parts[10]; // Assuming the description is in the 6th column (index 5)
